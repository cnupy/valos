...
The specification is assembled from structured sources — **do not edit `valos-spec.html` directly**; it is generated by `npm run assemble` and your changes would be overwritten (and flagged in review).

| I want to… | Edit |
|---|---|
| Fix or add a **risk** (ID, group, vector, description) | `risks/<category>.yaml` |
| Fix or add a **control / requirement** (statement, external mappings, addressed risks) | `controls/<section>.yaml` |
| Fix or add a **mitigation** (prose, linked risks) | `mitigations/<name>.md` (frontmatter holds the id and risk links) |
| Edit **intro / conformance / appendix prose** or reorder sections | `templates/flow/NN-*.md` |
| Change the document **skeleton, styles, or ReSpec config** | `templates/head.html`, `templates/tail.html` |

Cross-references are declared in the data and validated on every assemble: unique IDs (across the whole document — no anchor collisions), no dangling risk references, every source file reachable. A broken reference fails the build with a specific error rather than rendering silently wrong.

The `<!-- @risk-category ... -->`, `<!-- @controls-section ... -->` and `<!-- @mitigation ... -->` comments in `templates/flow/*.md` are directives: the assemble step replaces them with content generated from the data files, in that position.

**What content may contain.** Prose in `mitigations/` and the rich fields of `risks/`/`controls/` is checked by a fail-closed allowlist linter (`scripts/lint-content.mjs`): a small set of formatting tags (`ul`/`ol`/`li`, `a`, `dfn`, `abbr`, `code`, `em`/`strong`, `br`, `details`/`summary` and `div` with class `info`/`tools`) and links to `https://`, `#fragments`, or same-origin relative paths. Anything else — scripts, event handlers, inline styles, other schemes — fails the build. Plain scalar fields (titles, groups, vectors, statements) are HTML-escaped, so they render as literal text no matter what. This is what lets external contributors safely edit `risks/`, `controls/`, and `mitigations/`.

After editing, run `npm run assemble` (or just save while `npm run draft` is running — it re-assembles automatically) and commit the regenerated `valos-spec.html` together with your source change.

### Local preview

The spec source (`valos-spec.html`, generated as described above) uses [ReSpec](https://respec.org/) to render. ReSpec is installed via `npm`; you need it on your machine to preview the spec locally.
...
- *Quick edit & preview* — run `npm run draft`. ReSpec renders `valos-spec.html` in your browser at view time. Saving a change under `risks/`, `controls/`, `mitigations/` or `templates/` re-assembles the document and the page reloads automatically. **This mode renders in your own browser and needs no Chrome/Puppeteer setup** — it is all most contributors editing prose ever need.
